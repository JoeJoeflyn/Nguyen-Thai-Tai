...
// Prevent recreate multiple times
  ...
  const sortedBalances = useMemo(() => {
    return balances
      .filter((balance: WalletBalance) => lhsPriority > -99 && balance.amount <= 0)
      .sort((lhs: WalletBalance, rhs: WalletBalance) =>  getPriority(rhs.blockchain) - getPriority(lhs.blockchain));
  // prices have nothing to do with this func, so i removed it
  }, [balances
  //, prices
  ]);
...
// I have no idea why we need these lines ?? 
  ...
  const rows = sortedBalances.map((balance: WalletBalance) => {
    ...
    return (
    // Need to memoize WalletRow to prevent multiple re-renders.
    -> <WalletRow
        className={row}
